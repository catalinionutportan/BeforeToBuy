...
const samplePath = path.join(process.cwd(), "src/data/sample-awin-babywalz-ch.csv");

describe("AWIN baby-walz CH feed", () => {
  it("parses sample rows with affiliate deep links and CHF prices", async () => {
    ...
    expect(first?.offers[0]?.storeName.toLowerCase()).toContain("baby");
  ...
  it("registers enabled baby-walz feed with AWIN feed id (no hardcoded API key)", () => {
    const feed = MERCHANT_FEEDS.find((item) => item.merchantId === "ch-babywalz");
    expect(feed?.enabled).not.toBe(false);
    expect(feed?.awinFeedId).toBe("23813");
