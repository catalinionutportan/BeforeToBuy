...
export const STAGE_ZERO_MONETIZATION = {
  en: "BeforeToBuy.com currently has live merchant programmes via 2Performant in Romania (Rowenta, Scule365, evoMAG) and via AWIN in the United Kingdom (Seentat), each with a live product feed and affiliate deep links when Affiliate cookie consent is granted. Other merchants are added only after acceptance and feed wiring. Other markets remain planned. We only compare and redirect; checkout is always on the merchant site. We may earn a referral commission from the merchant or network — we do not add a BeforeToBuy fee to your price.",
  de: "BeforeToBuy.com hat derzeit Live-Händlerprogramme über 2Performant in Rumänien (Rowenta, Scule365, evoMAG) und über AWIN im Vereinigten Königreich (Seentat), jeweils mit Live-Produktfeed und Affiliate-Deep-Links bei Affiliate-Cookie-Zustimmung. Weitere Händler werden erst nach Freigabe und Feed-Anbindung ergänzt. Andere Märkte sind geplant. Wir vergleichen und leiten nur weiter; der Kaufabschluss erfolgt stets beim Händler. Wir können eine Vermittlungsprovision erhalten — ohne BeforeToBuy-Aufschlag.",
  fr: "BeforeToBuy.com a actuellement des programmes marchands live via 2Performant en Roumanie (Rowenta, Scule365, evoMAG) et via AWIN au Royaume-Uni (Seentat), chacun avec flux produit live et liens affiliés lorsque le consentement cookies Affilié est accordé. Les autres marchands sont ajoutés uniquement après acceptation et branchement du flux. Les autres marchés restent prévus. Nous comparons et redirigeons seulement ; le paiement a toujours lieu chez le marchand. Nous pouvons recevoir une commission — sans frais BeforeToBuy.",
  it: "BeforeToBuy.com ha attualmente programmi merchant live via 2Performant in Romania (Rowenta, Scule365, evoMAG) e via AWIN nel Regno Unito (Seentat), ciascuno con feed prodotti live e deep link affiliati con consenso cookie Affiliato. Altri merchant vengono aggiunti solo dopo accettazione e collegamento del feed. Altri mercati restano pianificati. Confrontiamo e reindirizziamo soltanto; il checkout è sempre sul sito del rivenditore. Possiamo ricevere una commissione — senza ricarico BeforeToBuy.",
  ro: "BeforeToBuy.com are momentan programe merchant live via 2Performant în România (Rowenta, Scule365, evoMAG) și via AWIN în Regatul Unit (Seentat), fiecare cu feed de produse live și deep link-uri afiliate când există consimțământ cookie Afiliat. Alte magazine se adaugă doar după acceptare și conectarea feed-ului. Alte piețe rămân planificate. Doar comparăm și redirecționăm; plata se face mereu pe site-ul magazinului. Putem primi comision de recomandare — fără adaos BeforeToBuy.",
} as const;

/** Networks with at least one live consented outbound program. */
export const AFFILIATE_NETWORKS_ACTIVE = [
  "2Performant Romania (Rowenta, Scule365, evoMAG)",
  "AWIN UK (Seentat)",
] as const;
...
export const DATA_PROCESSORS = [
  ...
  { name: "2Performant", purpose: "Affiliate tracking for Rowenta.ro, Scule365.ro and evoMAG.ro outbound links (with Affiliate consent)", region: "RO/EU" },
  ...
  { name: "evoMAG 2Performant product feed", purpose: "Product catalog and prices for evoMAG.ro (affiliate deep links from 2Performant My Feeds)", region: "RO/EU" },
