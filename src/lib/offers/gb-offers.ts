...
export async function getGbOffers(product: Product, userLocation: UserLocation, closestStore: any): Promise<Offer[]> {
  ...
  return [
    ...amazonOffers,
    {
      ...
      currency,
      inStock: true,
      deliveryTime: formatUi(HOME_UI[DEFAULT_LOCALE].clickAndCollectIn1Hour, {}),
      deliveryCost: 0,
      ...
      badge: closestStore ? formatUi(HOME_UI[DEFAULT_LOCALE].collectAtKmAway, { branchName: closestStore.branchName, distanceKm: closestStore.distanceKm }) : undefined,
    },
  ];
}
