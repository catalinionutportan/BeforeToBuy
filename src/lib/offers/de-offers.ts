...
export async function getDeOffers(product: Product, userLocation: UserLocation, closestStore: any): Promise<Offer[]> {
  ...
  return [
    ...amazonOffers,
    {
      ...
      badge: closestStore ? formatUi(`${HOME_UI[DEFAULT_LOCALE].pickupAt} {branchName} — {distanceKm} ${HOME_UI[DEFAULT_LOCALE].kmAway}`, { branchName: closestStore.branchName, distanceKm: closestStore.distanceKm }) : undefined,
    },
    {
      id: `${product.id}-otto`,
      storeName: "Otto.de",
