...
/**
 * First-impression block for users and partner reviewers.
 * Clarity over hype — keeps Beta honesty, no NEWS/editorial claims.
 */
export function MarketEntryHero({ locale }: MarketEntryHeroProps) {
  const ui = HOME_UI[locale];

  return (
    <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 text-white shadow-xs">
      <div
      ...
      <div className="relative p-6 sm:p-8 space-y-5">
        ...
        <div className="max-w-3xl space-y-3">
          ...
          <p className="text-xs sm:text-sm text-emerald-100/90 leading-relaxed border-l-2 border-emerald-400/50 pl-3">
            {ui.marketHeroDiff}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 pt-1">
          <a
            href="#browse-offers"
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs font-extrabold px-4 py-2.5 transition-colors"
          >
          ...
          <Link
            href="/legal"
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/5 hover:bg-white/10 text-white text-xs font-bold px-4 py-2.5 transition-colors"
          >
            <Scale className="h-3.5 w-3.5 text-emerald-300" aria-hidden="true" />
            {ui.marketHeroCtaLegal}
          </Link>
          <p className="text-[11px] text-slate-400 sm:ml-auto">{ui.marketHeroPartnerNote}</p>
        </div>
      </div>
    </section>
  );
