/**
 * Import Reifen.com CH (AWIN) catalogue into Supabase.
 *
 * Prefers a direct feed download (needs AWIN_API_KEY or AWIN_FEED_URL_CH_REIFENCOM
 * in .env.local). Falls back to the live API catalogue when the feed path fails.
 *
 * Usage: node --env-file=.env.local --import tsx src/scripts/import-ch-reifencom.ts
 */
...
const PAGE_SIZE = 200;
const MERCHANT_ID = "ch-reifencom";
...
async function fetchFromFeed(): Promise<ApiProduct[] | null> {
  try {
    ...
    console.warn(
      "Direct AWIN feed unavailable (set AWIN_API_KEY or AWIN_FEED_URL_CH_REIFENCOM):",
...
async function writeCatalogue(products: ApiProduct[]) {
  ...
  console.log(`Done. CH products=${pc}, Reifen.com offers=${oc}, with description=${withDesc}`);
}

async function main() {
  console.log("Importing Reifen.com CH into Supabase...");
