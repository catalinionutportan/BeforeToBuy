/**
 * Import baby-walz CH (AWIN) catalogue into Supabase.
 *
 * Prefers a direct feed download (needs AWIN_API_KEY or AWIN_FEED_URL_CH_BABYWALZ
 * in .env.local). Falls back to the live API catalogue when the feed path fails.
 *
 * Usage: node --env-file=.env.local --import tsx src/scripts/import-ch-babywalz.ts
 */
...
async function fetchFromFeed(): Promise<ApiProduct[] | null> {
  try {
    ...
    console.warn(
      "Direct AWIN feed unavailable (set AWIN_API_KEY or AWIN_FEED_URL_CH_BABYWALZ):",
...
async function writeCatalogue(products: ApiProduct[]) {
  ...
  console.log(`Done. CH products=${pc}, baby-walz offers=${oc}, with description=${withDesc}`);
}

async function main() {
  console.log("Importing baby-walz CH into Supabase...");
