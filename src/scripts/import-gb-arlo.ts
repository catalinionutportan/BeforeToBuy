/**
 * Import Arlo Security UK (AWIN feed 112803) into Supabase.
 *
 * Needs AWIN_API_KEY (or AWIN_FEED_URL_GB_ARLO) in .env.local.
 * Catalogue is small (~85 SKUs) and also loads on the request path.
 *
 * Usage: node --env-file=.env.local --import tsx src/scripts/import-gb-arlo.ts
 */
...
async function main() {
  console.log("Importing Arlo Security UK into Supabase...");
  ...
  console.log(`Done. GB visible products=${pc}, Arlo offers=${oc}`);
